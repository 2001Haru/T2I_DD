...
B0_RUN_ROOT="${B0_RUN_ROOT:-./conditioning_interface_matrix_runs/b0_conditioning_length_v2}"
...
python experiments/text_supervision_factorial/run_b0_conditioning_length.py \
  ...
  --matched-generation-seeds ${MATCHED_GENERATION_SEEDS:-0 1 2 3 4} \
  --control-generation-seeds ${CONTROL_GENERATION_SEEDS:-0 1 2} \
  --ipc 10 \
  --strength 0.7 \
  --classifier-repeats "${CLASSIFIER_REPEATS:-2}" \
  --guidance-diagnostic-timesteps ${GUIDANCE_DIAGNOSTIC_TIMESTEPS:-200 500 800} \
