...
def get_args():
    ...
    parser.add_argument(
        "--use_cluster_captions", action="store_true", default=False,
        help="Use precomputed cluster captions to construct the SDXL prompt."
    )
    parser.add_argument(
        "--cluster_caption_model_path", type=str, default="llava-hf/llava-1.5-7b-hf",
        help="Hugging Face model ID or local path for the LLaVA caption model."
    )
    parser.add_argument(
        "--cluster_caption_file", type=str, default=None,
        help="Optional caption JSON path. Defaults to cluster_captions.json in the experiment directory."
    )
    parser.add_argument(
        "--cluster_caption_device", type=str, default="cuda:0",
        help="Device used by the offline LLaVA captioning stage."
    )
    parser.add_argument(
        "--cluster_caption_max_new_tokens", type=int, default=64,
    ...
    parser.add_argument(
        "--overwrite_cluster_captions", action="store_true", default=False,
        help="Regenerate the caption manifest even when it already exists."
    )
    parser.add_argument(
        "--generated_images_dirname", type=str, default=None,
        help="Optional output subdirectory for generated images."
    )
    ...
    if args.use_cluster_captions:
        ...
        missing_template_fields = [
            field for field in required_template_fields if field not in args.cluster_caption_prompt_template
        ]
